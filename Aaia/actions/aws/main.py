...
def main(config,args):
	if args.action  and args.action !="main":
		if args.action=="help":
			...
			return
		try:
			library = importlib.import_module(f"{__package__}.{args.action}.main")
			...
			if args.name == "all" and args.action != "clean_db":
				...
				library.main(config,args)
			del sys.modules[f"{__package__}.{args.action}.main"]
...
def help():
	...
	for module in os.listdir(os.path.dirname(__file__)):
		...
		if os.path.isdir(absolute_path) and not module.startswith("_"):
			library = importlib.import_module(f"{__package__}.{module}.main")
			library.help()
			del sys.modules[f"{__package__}.{module}.main"]
